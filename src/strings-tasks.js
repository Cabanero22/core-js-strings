...
function getStringLength(value) {
  if (Boolean(value) === false || typeof value === typeof 1) {
    return 0;
  }
  return value.length;
}
...
function isString(value) {
  if (typeof value === typeof 'string' || value instanceof String) return true;
  return false;
}
...
function removeFirstOccurrences(str, value) {
  ...
  const slice = str.slice(0, start) + str.slice(end, str.length);
  return slice;
}
...
function isPalindrome(str) {
  const strLower = str.toLowerCase().replaceAll(/[!,.?' ']/g, '');
  let sum = '';
  for (let i = strLower.length - 1; i >= 0; i -= 1) {
    sum = `${sum}${strLower[i]}`;
  }
  if (strLower === sum) return true;
  return false;
}
...
function extractNameFromTemplate(value) {
  const result = value.replaceAll('Hello,', '').replaceAll('!', '');
...
function encodeToRot13(str) {
  ...
  const result = str.replace(
    /[a-z]/gi,
    (elementLetter) => rot13[alphabet.indexOf(elementLetter)]
  );
  return result;
}
